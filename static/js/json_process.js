...
function Process(raw_json_container_id, formatted_json_container_id){
  SetTab();
  window.IsCollapsible = false;
  var json = $id(raw_json_container_id).value;
  var html = "";
  try{
    if(json == "") json = "\"\"";
    var obj = eval("["+json+"]");
    html = ProcessObject(obj[0], 0, false, false, false);
    $id(formatted_json_container_id).innerHTML = "<PRE class='CodeContainer'>"+html+"</PRE>";
  }catch(e){
    alert("JSON数据格式不正确:\n"+e.message);
    $id(formatted_json_container_id).innerHTML = "";
  }
}
...
function SetTab(){
  ...
  window.TAB = MultiplyString(4, window.SINGLE_TAB);
...
function SelectAllClicked(area_id){
  ...
  if(!!range.selectNode)
    ...
    range.selectNode($id(area_id));

  else if(range.moveToElementText)

    range.moveToElementText($id(area_id));

  

  if(!!range.select)

    range.select($id(area_id));
